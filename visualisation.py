...
def afficher_sequence(resultats, labels, L0):
    ...
    axes = np.array(axes).reshape(-1)  # aplatit en une seule liste, quel que soit nrows
    ...
    for ax in axes[len(instants):]:  # cache les panneaux vides si le compte
        ax.axis("off")              # d'instants n'est pas un multiple de 3
